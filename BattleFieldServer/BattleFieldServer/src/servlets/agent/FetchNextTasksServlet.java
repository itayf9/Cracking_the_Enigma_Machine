...
public class FetchNextTasksServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (isValidSession) {
            ...
            if (uboatName == null) {
                ...
                DTOtasks tasksStatus = engine.getNextTasks(usernameFromSession, allieName, uboatName);
                if (!tasksStatus.isSucceed()) {
                    resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                } else {
                    resp.setStatus(HttpServletResponse.SC_OK);
                }
                resp.getWriter().println(gson.toJson(tasksStatus));
            }
        }
    }
}
